...
#define FS_REG_BASE     "SOFTWARE\\Lockheed Martin\\Prepar3D v3"
...
#define KEY_EYEPOINT_SET_LATERAL                (KEY_ID_MIN + 1452)
#define KEY_EYEPOINT_SET_VERTICAL               (KEY_ID_MIN + 1453)
#define KEY_EYEPOINT_SET_LONGITUDINAL           (KEY_ID_MIN + 1454)
...
#define KEY_AXIS_PAN_VELOCITY_PITCH              (KEY_ID_MIN + 1529)
#define KEY_AXIS_PAN_VELOCITY_HEADING            (KEY_ID_MIN + 1530)
#define KEY_AXIS_PAN_VELOCITY_TILT               (KEY_ID_MIN + 1531)
...
#define KEY_WEAPONS_FIRE                         (KEY_ID_MIN + 1621)
#define KEY_WEAPONS_CYCLE_TYPE                   (KEY_ID_MIN + 1622)
#define	KEY_WEAPONS_CYCLE_SUBTYPE                (KEY_ID_MIN + 1623)
#define	KEY_WEAPONS_RESET                        (KEY_ID_MIN + 1624)
...
typedef LPTSTR      PSTRINGZ; // DEPRECATED
typedef LPCTSTR     PCSTRINGZ; // DEPRECATED
...
public:
    ...
    virtual bool GetParameter(const TCHAR* szParName, const TCHAR** pszValue) const = 0;
    virtual bool GetParameter(const TCHAR* szParName, FLOAT64* pdValue) const = 0;
    virtual bool GetParameter(const TCHAR* szParName, SINT32* piValue) const = 0;
    virtual bool GetParameter(const TCHAR* szParName, bool* pbValue) const = 0;
    virtual bool SetParameterId(const TCHAR* szParName, SINT32 id, PARAMETER_TYPE type) const = 0;
};
...
public:
    virtual ULONG AddRef () = 0;
    virtual ULONG Release () = 0;
    virtual IGaugeCCallback* QueryInterface (LPCTSTR pszInterface) = 0;
    virtual void Update () = 0;
    virtual bool GetPropertyValue (SINT32 id, FLOAT64* pValue) = 0;
    virtual bool GetPropertyValue (SINT32 id, LPCTSTR* pszValue) = 0;
    virtual bool SetPropertyValue (SINT32 id, FLOAT64 value) = 0;
    virtual bool SetPropertyValue (SINT32 id, LPCTSTR szValue) = 0;
    virtual IGaugeCDrawable* CreateGaugeCDrawable (SINT32 id, const IGaugeCDrawableCreateParameters* pParameters) = 0;
};

#define ISERIALIZABLE_GAUGECCALLBACK_NAME  TEXT("ISerializableGaugeCCallback")
...
public:
    virtual ULONG AddRef () = 0;
    virtual ULONG Release () = 0;
    virtual IAircraftCCallback* QueryInterface (LPCTSTR pszInterface) = 0;
...
public:
    virtual ULONG AddRef () = 0;
    virtual ULONG Release () = 0;
    virtual IPanelCCallback* QueryInterface (LPCTSTR pszInterface) = 0;
    virtual UINT32 GetVersion () = 0;
    virtual IAircraftCCallback*  CreateAircraftCCallback (UINT32 ContainerID) = 0;
    virtual bool  ConvertStringToProperty (LPCTSTR keyword, SINT32* pID) = 0;
    virtual bool  ConvertPropertyToString (SINT32 id, LPCTSTR* pKeyword) = 0;
...
typedef struct ELEMENT_DXGAUGE			*PELEMENT_DXGAUGE,				**PPELEMENT_DXGAUGE;
...
typedef FLOAT64 FSAPI	DXGAUGE_UPDATE_CALLBACK(PELEMENT_DXGAUGE pelement);
typedef					DXGAUGE_UPDATE_CALLBACK					*PDXGAUGE_UPDATE_CALLBACK;
...
typedef struct DXGAUGEHDR : public GAUGEHDR
{
	enum DXVERSION {UNKNOWN=0, DX9=9};
	DXVERSION getDeviceVers(void);
	void* getDevice(void);
	HWND getDeviceWindow (void);
} DXGAUGEHDR, *PDXGAUGEHDR, **PPDXGAUGEHDR;
...
typedef struct {
        ...
        FLOAT64         (FSAPI *numeric_callback) (FLOAT64 number, ID id, LPCTSTR string, MODULE_VAR *source_var, PGAUGEHDR gauge);
        ID              (FSAPI *id_callback) (FLOAT64 number, ID id, LPCTSTR string, MODULE_VAR *source_var, PGAUGEHDR gauge);
        LPCTSTR       (FSAPI *string_callback) (FLOAT64 number, ID id, LPCTSTR string, MODULE_VAR *source_var, PGAUGEHDR gauge);
} MOUSE_ARG;
...
typedef struct  PANELS
{
    ...
    void        (FSAPI *register_var_by_name) ( PVOID var, VAR_TYPE var_type, LPTSTR name );  
    void        (FSAPI *initialize_var) ( PMODULE_VAR module_var ); 
    void        (FSAPI *initialize_var_by_name) ( PMODULE_VAR module_var, LPTSTR name );  
    void        (FSAPI *lookup_var) ( PMODULE_VAR module_var ); 
    void        (FSAPI *unregister_var_by_name) ( LPTSTR name );  
    ...
    ID          (FSAPI *check_named_variable) (LPCTSTR name);
    ID          (FSAPI *register_named_variable) (LPCTSTR name);
    ...
    LPCTSTR   (FSAPI *get_name_of_named_variable) (ID id);
    void        (FSAPI *reserved29) (void); 
    LPCTSTR   (FSAPI *panel_resource_string_get) (ID32 id);   
    ...
    void        (FSAPI *set_gauge_flags) (LPCTSTR name, FLAGS32 newflags);        // newlags is GAUGE_FLAG_NORMAL, GAUGE_FLAG_HIDDEN... 
    FLAGS32     (FSAPI *get_gauge_flags) (LPCTSTR name);
    BOOL        (FSAPI *gauge_calculator_code_precompile) (LPCTSTR* pCompiled, UINT32* pCompiledSize, LPCTSTR source);
    BOOL        (FSAPI *execute_calculator_code) (LPCTSTR code, FLOAT64* fvalue, SINT32* ivalue, LPCTSTR* svalue);
    BOOL        (FSAPI *format_calculator_string) (LPTSTR result, UINT32 resultsize, LPCTSTR format);
    BOOL        (FSAPI *reserved32) (void);
    BOOL        (FSAPI *reserved33) (void);
    ENUM        (FSAPI *get_units_enum) (LPCTSTR unitname);
    ENUM        (FSAPI *get_aircraft_var_enum) (LPCTSTR simvar);
    FLOAT64     (FSAPI *aircraft_varget) (ENUM simvar, ENUM units, SINT32 index);
    BOOL        (FSAPI *panel_register_c_callback) (LPCTSTR name, IPanelCCallback* pcallback);
    IPanelCCallback* (FSAPI *panel_get_registered_c_callback) (LPCTSTR name);
    IAircraftCCallback* (FSAPI *panel_get_aircraft_c_callback) (LPCTSTR name);
    ...
    LPCTSTR   (FSAPI *get_event_description) (ID32 event_id);
    LPCTSTR   (FSAPI *get_event_token_string) (ID32 event_id);
